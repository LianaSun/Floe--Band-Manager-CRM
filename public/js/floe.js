...
$(document).ready(function() {
  ...
  function createNewRow(post) {
    var formattedDate = new Date(post.date);
    var formattedTime = post.time;
    formattedDate = moment(formattedDate).format("LL");
    // formattedTime = moment(formattedTime).format("LT")
    var newPostCard = $("<div>");
    ...
    var newPostArtist = $("<h5>");
    var newPostVenue = $("<h4>");
    newPostArtist.text("Booked for: " + post.Artist.name);
    newPostArtist.css({
      float: "right",
      color: "blue",
      "margin-top": "-10px"
    ...
    newPostCardBody.addClass("card-body");
    newPostTitle.text(post.name + " ");
    newPostVenue.text(post.Venue.name);
    newPostDate.text(formattedDate);
    newPostTime.text(formattedTime);
    newPostTitle.append(newPostDate).append(newPostTime);
    newPostCardHeading.append(deleteBtn);
    newPostCardHeading.append(editBtn);
    newPostCardHeading.append(newPostTitle);
    newPostCardHeading.append(newPostArtist);
    newPostCardHeading.append(newPostVenue);
    newPostCard.append(newPostCardHeading);
    newPostCard.append(newPostCardBody);
    newPostCard.data("post", post);
    return newPostCard;
